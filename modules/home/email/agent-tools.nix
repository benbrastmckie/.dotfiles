  ...
  home.packages = [
    ...
    (pkgs.writeShellScriptBin "email-census" (mkPreamble {
      ...
      for y in $(seq $((CUR_YEAR - 4)) "$CUR_YEAR"); do
        n=$(notmuch count "folder:Gmail and date:$y-01-01..$y-12-31" 2>/dev/null || echo 0)
        printf "%-10s %s\n" "$y" "$n"
      done
      echo ""
      ...
      himalaya envelope list -f INBOX -o json -s 10 2>/dev/null \
        | jq -c '.[] | {id, subject, from: .from.addr, date}' 2>/dev/null \
        || echo "(himalaya envelope list unavailable — is the gmail account configured?)"
    ''))
    ...
    (pkgs.writeShellScriptBin "email-classify" (mkPreamble {
      ...
      extraHelp = ''
        Classify flags:
          [QUERY]                 notmuch query to classify (default: "folder:Gmail" = INBOX)
    ...
    (pkgs.writeShellScriptBin "email-unsubscribe-extract" (mkPreamble {
      ...
      extraHelp = ''
        Flags:
          [QUERY]        notmuch query to scan (default: "folder:Gmail" = INBOX)
