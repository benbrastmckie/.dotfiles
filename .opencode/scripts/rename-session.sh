...
while IFS= read -r line; do
  ...
  # Keep first non-serve match as fallback
  [ -z "$port" ] && port="$p"
done < <(ss -tlnp 2>/dev/null | grep opencode)

if [ -z "$port" ]; then
  echo "WARN: No OpenCode TUI found, skipping rename" >&2
  exit 0
fi
...
if [ -z "$busy_id" ]; then
  # Fallback: use the most recently updated session
...
if [ -z "$busy_id" ]; then
  echo "WARN: Could not determine current session, skipping rename" >&2
  exit 0
fi
...
echo "OK: Session renamed to '${title}'"
